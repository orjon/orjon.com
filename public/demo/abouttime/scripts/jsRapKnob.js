;(function ($) {
  $.fn.jsRapKnob = function (options) {
    return this.each(function () {
      ...
      $('<div>')
        .addClass('rapKnobCaption')
        .text(this.opt.caption)
        .appendTo($(this))
      let w = $(this).width()
      // $(this).css({height: w})
      ...
      function Update(e) {
        ...
        if (p < 0) p = p > -0.125 ? 1 : 1 + ((p + 0.125) * 0.5) / 0.375
        else p = p < 0.125 ? 0 : ((p - 0.125) * 0.5) / 0.375
        base.SetPosition(p)
      }
      ...
      this.SetPosition = function (p) {
        ...
        inBar.css(
          'transform',
          'rotate(' + d + 'deg) translate(0,-' + w / 2 + 'px)'
        )
        if (this.opt.onChange) this.opt.onChange.call(this, p)
      }
