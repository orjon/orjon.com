...
export const ImageCarousel = ({ images, shadow = false }: { images: string[], shadow?: boolean }) => {
  ...
  const slides = images.map((image, index) =>
    <div key={index} className='embla__slide flex-[0_0_100%] cursor-w-resize px-1 pb-4 flex items-center justify-center'>
      <Image src={image} alt={`Image ${index}`} width={1500} height={500} className={`max-h-[50vh] mx-auto object-contain ${shadowClass}`} />
    </div>
  )
  ...
  return (
    <div className="ImageCarousel mx-auto embla flex flex-col items-center">
